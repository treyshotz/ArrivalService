...
class FlightsClientTest {
  ...
  void testConvertAndSaveMultiple() {
    ...
    client.convertAndSave(new MessageResponse().setMessage(json).setStatusCode(statusCode));
    ;

    verify(instance, times(1)).insertStates(any(List.class));
  }
  ...
  @Test
  void testRetrieveArrivalsAirportIntervalSingle() throws IOException, InterruptedException {
    var httpMock = mock(HttpClient.class);
    var resp = mock(HttpResponse.class);
  
    doReturn(resp).when(httpMock).send(any(), any());
    ...
    verify(httpMock, times(1)).send(any(), any());
  }

  @Test
  void testRetrieveArrivalsAirportIntervalMultiple() throws IOException, InterruptedException {
