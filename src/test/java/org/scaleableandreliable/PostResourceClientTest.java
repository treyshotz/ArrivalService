...
class PostResourceClientTest {
  ...
  void testRetrieveArrivalsAirportIntervalSingle() throws ExecutionException, InterruptedException {
    ...
    doReturn(new CompletableFuture<>().minimalCompletionStage()).when(httpClient).sendAsync(any(), any());
  ...
  @Test
  void testRetrieveArrivalsAirportIntervalMultiple() throws ExecutionException, InterruptedException {
    var httpClient= mock(HttpClient.class);
    doReturn(new CompletableFuture<>().minimalCompletionStage()).when(httpClient).sendAsync(any(), any());
    client.httpClient = httpClient;
